...
export async function POST(request: NextRequest) {
  try {
    ...
    const keyDoc = await keys.findOne({ key, isActive: true })
    if (!keyDoc) {
      return NextResponse.json(
        {
          status: false,
    ...
    if (isFirstUse) {
      ...
      try {
        ...
        await activities.insertOne({
          ...
          details: `Key activated: ${key.substring(0, 8)}... | Expires: ${finalExpiryDate}`,
          userId: 'system',
    ...
    try {
      ...
      await activities.insertOne({
        action: 'user_login',
        details: `Login successful for key: ${key.substring(0, 8)}...`,
        userId: uuid,
        ipAddress: request.headers.get('x-forwarded-for') || request.headers.get('x-real-ip') || 'unknown',
    ...
    try {
      ...
      await analytics.insertOne({
        ...
        uuid: uuid,
        createdBy: keyDoc.createdBy || 'unknown',
        timestamp: now.toISOString(),
        ipAddress: request.headers.get('x-forwarded-for') || request.headers.get('x-real-ip') || 'unknown',
    ...
    // Get mod name from api_keys collection
    let modname = ""
    try {
      const apiKeys = db.collection('api_keys')
